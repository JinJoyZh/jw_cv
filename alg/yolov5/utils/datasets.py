...
class LoadImages:  # for inference
    def __init__(self, path, img_size=640, defog_flag=0):
...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', img_size=640, defog_flag=0):
        self.mode = 'stream'
        self.img_size = img_size
        self.defog_flag = defog_flag
        ...
        for i, s in enumerate(sources):
            ...
            capture_target = eval(s) if s.isnumeric() else s
    ...
    def __next__(self):
        self.count += 1
        img0 = self.imgs.copy()
        img0 = list(filter(lambda element: element is not None, img0))
        if cv2.waitKey(1) == ord('q'):  # q to quit
            cv2.destroyAllWindows()
            raise StopIteration
        ...
        if len(img) == 0:
            return self.sources, None, None, self.cap
        # Stack
        img = np.stack(img, 0)
...
def img2label_paths(img_paths):
    ...
    sa, sb = os.sep + 'images' + os.sep, os.sep + 'txt' + os.sep  # /images/, /labels/ substrings
